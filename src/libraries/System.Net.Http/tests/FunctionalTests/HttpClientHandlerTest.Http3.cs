    ...
    public sealed class HttpClientHandlerTest_Http3 : HttpClientHandlerTestBase
    {
        ...
        public async Task ClientSettingsWebTransportReceived_Success()
        {
            ...
            Task serverTask = Task.Run(async () =>
            {
                ...
                await using (stream)
                {
                    ...
                    byte[] bytes = new byte[50];// = Encoding.ASCII.GetBytes(s);
        ...
        public async Task ClientSettingsWebTransportAPI()
        {
            ...
            Task serverTask = Task.Run(async () =>
            {
                ...
                headers.Add(header);
                headers.Append(header);
                Console.WriteLine("I send headers 1");

                (Http3LoopbackStream settingsStream, Http3LoopbackStream stream) = await connection.AcceptControlAndRequestStreamAsync();
                Console.WriteLine("I send headers 2");

                await using (settingsStream)
                await using (stream)
                {
                    Console.WriteLine("I send headers 3");

                    Assert.Equal(1, connection.EnableWebtransport);
                    await stream.ReadRequestDataAsync(false);
                    Console.WriteLine("I send headers 4.1");
                    await stream.SendResponseAsync(HttpStatusCode.OK, headers, "", false);
                    Console.WriteLine("I send headers 4");
